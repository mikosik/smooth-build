...
public class BSubExprImpl implements BSubExpr {
  private final BExpr owner;
  private final BExpr expr;
  private final String name;

  public BSubExprImpl(BExpr owner, BExpr expr, String name) {
    this.owner = owner;
    this.expr = expr;
    this.name = name;
  }
  ...
  public BExpr asExpr(BType expectedEvaluationType) throws BExprDbException {
    owner.checkSubExprEvaluationType(name, expr.evaluationType(), expectedEvaluationType);
    return expr;
  }
  ...
  public <T extends BExpr> T asInstanceOf(Class<T> clazz) throws BExprDbException {
    return owner.castSubExpr(expr, name, clazz);
  }
}
