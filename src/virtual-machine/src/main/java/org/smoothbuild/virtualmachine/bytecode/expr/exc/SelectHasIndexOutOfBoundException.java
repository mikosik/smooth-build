...
public class SelectHasIndexOutOfBoundException extends DecodeExprException {
  public SelectHasIndexOutOfBoundException(Hash hash, BSelectKind kind, int index, int size) {
    super(buildMessage(hash, kind, index, size));
  }

  private static String buildMessage(Hash hash, BSelectKind kind, int index, int size) {
