...
public class BChoiceTest extends VmTestContext {
  @Test
  void setting_value_with_wrong_type_throws_exception() throws BytecodeException {
    var type = bChoiceType(bStringType(), bBlobType());
    assertCall(() -> exprDb().newChoice(type, bInt(0), bBlob()))
        .throwsException(
            new IllegalArgumentException("`value.type()` should be `String` but is `Blob`."));
  }
  ...
  @Test
  void setting_value_to_null_throws_exception() {
