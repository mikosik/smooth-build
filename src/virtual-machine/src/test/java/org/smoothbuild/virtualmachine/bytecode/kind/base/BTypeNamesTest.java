...
public class BTypeNamesTest extends VmTestContext {
  ...
  @Test
  void variant_type_name_with_single_alternative() throws Exception {
    assertThat(variantTypeName(list(bIntType()))).isEqualTo("{Int|}");
  }

  @Test
  void variant_type_name_with_multiple_alternatives() throws Exception {
    assertThat(variantTypeName(list(bBlobType(), bIntType()))).isEqualTo("{Blob|Int}");
  }
}
