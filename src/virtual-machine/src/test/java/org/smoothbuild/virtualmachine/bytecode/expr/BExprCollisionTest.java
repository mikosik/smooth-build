...
public class BExprCollisionTest extends TestingVirtualMachine {
  ...
  private List<BExpr> exprBs() throws Exception {
    ...
    return List.of(
        ...
        bInt(0),
        bInt(1),
        bNativeFunc(bFuncType(bIntType()), blob1, bString("binary name"), bBool(true)),
        bNativeFunc(bFuncType(bIntType()), blob1, bString("binary name"), bBool(false)),
        bNativeFunc(bFuncType(bIntType()), blob1, bString("binary name 2"), bBool(true)),
        bNativeFunc(bFuncType(bIntType()), blob2, bString("binary name"), bBool(true)),
        bNativeFunc(bFuncType(bStringType()), blob1, bString("binary name"), bBool(true)),
        bString("abc"),
        bString("def"),
        bTuple(),
        bTuple(bInt(0)),
        ...
        bCall(bNativeFunc(bFuncType(bIntType(), bIntType())), bInt(1)),
        bCall(bNativeFunc(bFuncType(bIntType(), bIntType())), bInt(2)),
        bCall(bNativeFunc(bFuncType(bIntType(), bStringType())), bInt(1)),
        bCombine(),
        bCombine(bInt(0)),
        bCombine(bInt(1)),
