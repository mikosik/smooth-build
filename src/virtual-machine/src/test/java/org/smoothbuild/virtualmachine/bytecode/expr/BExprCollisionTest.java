...
public class BExprCollisionTest extends VmTestContext {
  ...
  private List<BExpr> exprBs() throws Exception {
    ...
    return List.of(
        ...
        bBool(false),
        bBool(true),
        bVariant(bVariantType(), bInt(0), bString("7")),
        bVariant(bVariantType(), bInt(1), bInt(7)),
        bConstructVariant(bVariantType(), bInt(0), bString("7")),
        bConstructVariant(bVariantType(), bInt(1), bInt(7)),
        bSwitch(bVariant(), bConstructTuple(bs2iLambda(), bi2iLambda(1))),
        ...
        bConstructArray(bIntType(), bInt(1)),
        bConstructArray(bIntType(), bInt(2)),
        bConstructArray(bIntArrayType(), bArray(bIntType())),
        bArrayGet(bArray(bInt(1)), bInt(0)),
        bArrayGet(bArray(bInt(1)), bInt(1)),
        bArrayGet(bArray(bInt(1), bInt(2)), bInt(0)),
        bRef(bIntType(), 0),
        bRef(bIntType(), 1),
        bRef(bStringType(), 0),
        bTupleGet(bTuple(bInt(1), bString("a")), bInt(0)),
        bTupleGet(bTuple(bInt(1), bString("a")), bInt(1)),
        bTupleGet(bTuple(bInt(1), bString("b")), bInt(0)));
  }
}
