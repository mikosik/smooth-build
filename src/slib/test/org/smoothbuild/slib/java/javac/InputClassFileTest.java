...
public class InputClassFileTest extends TestingContextImpl {
  ...
  public void file_without_class_extension_is_forbidden() {
    assertCall(() -> new InputClassFile(file(path("abc"))))
        .throwsException(IllegalArgumentException.class);
  }

  @Test
  public void input_class_files_with_equal_paths_but_different_content_are_equal() {
    assertThat(new InputClassFile(file(path, ByteString.encodeUtf8("abc"))))
        .isEqualTo(new InputClassFile(file(path, ByteString.encodeUtf8("def"))));
  }

  @Test
  public void input_class_files_with_different_paths_are_not_equal() {
    assertThat(new InputClassFile(file(path("a/b/MyClass.class"), bytes)))
        .isNotEqualTo(new InputClassFile(file(path("a/b/OtherClass.class"), bytes)));
  }

  @Test
  public void class_in_default_package_has_empty_package() {
    InputClassFile inputClassFile = new InputClassFile(file(path("Myclass.class")));
  ...
  public void aPackage_returns_class_package() {
    InputClassFile inputClassFile = new InputClassFile(file(path("a/b/Myclass.class")));
    assertThat(inputClassFile.aPackage())
        .isEqualTo("a.b");
  }
  ...
  public void aPackage_for_inner_class_returns_package_of_enclosing_class() {
    InputClassFile inputClassFile = new InputClassFile(file(path("a/b/MyClass$Inner.class")));
    assertThat(inputClassFile.aPackage())
        .isEqualTo("a.b");
  }
