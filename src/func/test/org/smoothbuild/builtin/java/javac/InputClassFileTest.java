...
public class InputClassFileTest {
  ...
  public void input_class_files_with_equal_paths_are_equal() throws Exception {
    when(new InputClassFile(file(objectsDb, path, "content")));
    thenReturned(new InputClassFile(file(objectsDb, path, "other content")));
  }

  @Test
  public void input_class_files_with_different_paths_are_not_equal() throws Exception {
    when(new InputClassFile(file(objectsDb, path("a/b/MyClass.class"), "content")));
    thenReturned(not(new InputClassFile(file(objectsDb, path("a/b/OtherClass.class"), "content"))));
  }

  @Test
  public void class_in_default_package_has_empty_package() throws Exception {
    given(inputClassFile = new InputClassFile(file(objectsDb, path("Myclass.class"))));
  ...
  public void aPackage_returns_class_package() throws Exception {
    given(inputClassFile = new InputClassFile(file(objectsDb, path("a/b/Myclass.class"))));
    when(inputClassFile).aPackage();
    thenReturned("a.b");
  }
  ...
  public void aPackage_for_inner_class_returns_package_of_enclosing_class() throws Exception {
    given(inputClassFile = new InputClassFile(file(objectsDb, path("a/b/MyClass$Inner.class"))));
    when(inputClassFile).aPackage();
    thenReturned("a.b");
  }
  ...
  private InputClassFile inputClassFile(String path) {
    return new InputClassFile(file(objectsDb, path(path)));
  }
}
