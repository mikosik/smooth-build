...
public abstract class AbstractSoftTerminationExecutorTestSuite {
  ...
  @Test
  void submitted_runnable_gets_executed() throws Exception {
  ...
  @Test
  void empty_executor_can_be_terminated() throws Exception {
    executor.terminate();
    executor.awaitTermination();
  }

  @Test
  void executor_can_be_terminated_twice() throws Exception {
    executor.terminate();
    executor.terminate();
    executor.awaitTermination();
  }
  ...
  @Test
  void runnable_can_terminate_executor() throws Exception {
    executor.enqueue(executor::terminate);
    executor.awaitTermination();
  }

  @Test
  void runnable_can_enqueue_another_runnable() throws Exception {
    executor.enqueue(() -> executor.enqueue(executor::terminate));
    executor.awaitTermination();
  }

  @Test
  void client_can_terminate_executor() throws Exception {
    executor.terminate();
    executor.awaitTermination();
  }

  @Test
  void runnable_submitted_after_termination_is_ignored() throws InterruptedException {
    Completable completable = new Completable();
    executor.terminate();
    executor.enqueue(completable);
    executor.awaitTermination();
