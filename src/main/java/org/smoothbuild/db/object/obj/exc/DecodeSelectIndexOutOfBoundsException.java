...
public class DecodeSelectIndexOutOfBoundsException extends DecodeObjException {
  public DecodeSelectIndexOutOfBoundsException(Hash hash, TypeH type, int index, int size) {
    super(buildMessage(hash, type, index, size));
  }

  private static String buildMessage(Hash hash, TypeH type, int index, int size) {
