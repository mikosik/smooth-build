...
public class DecodeTypeNodeException extends DecodeTypeException {
  public DecodeTypeNodeException(Hash hash, SpecKindH kind, String path, String message) {
    super(buildMessage(hash, kind, path, message));
  }

  public DecodeTypeNodeException(Hash hash, SpecKindH kind, String path) {
    super(buildMessage(hash, kind, path, null));
  }

  public DecodeTypeNodeException(Hash hash, SpecKindH kind, String path, int index, Throwable e) {
    this(hash, kind, path + "[" + index + "]", e);
  }

  public DecodeTypeNodeException(Hash hash, SpecKindH kind, String path, Throwable e) {
    super(buildMessage(hash, kind, path, null), e);
  }

  private static String buildMessage(Hash hash, SpecKindH kind, String path, String message) {
