...
public class DecodeTypeNodeException extends DecodeTypeException {
  public DecodeTypeNodeException(Hash hash, ObjKind kind, String path, String message) {
    super(buildMessage(hash, kind, path, message));
  }

  public DecodeTypeNodeException(Hash hash, ObjKind kind, String path) {
    super(buildMessage(hash, kind, path, null));
  }

  public DecodeTypeNodeException(Hash hash, ObjKind kind, String path, int index, Throwable e) {
    this(hash, kind, path + "[" + index + "]", e);
  }

  public DecodeTypeNodeException(Hash hash, ObjKind kind, String path, Throwable e) {
    super(buildMessage(hash, kind, path, null), e);
  }

  private static String buildMessage(Hash hash, ObjKind kind, String path, String message) {
