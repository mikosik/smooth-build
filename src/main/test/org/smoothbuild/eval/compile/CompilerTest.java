...
public class CompilerTest extends TestingContext {
  ...
  class _compiling {
    ...
    public void call_polymorphic() {
      ...
      var v = oVarTB("A");
      assertConversion(identity, call,
          callB(stringTB(), funcB(v, list(v), paramRefB(close(v), 0)), stringB("abc")));
    }
  ...
  private Compiler newCompiler(DefsS defs, FileLoader fileLoader) {
    return new Compiler(byteCodeFactory(), typingB(), defs, typeShConv(), fileLoader);
  }
