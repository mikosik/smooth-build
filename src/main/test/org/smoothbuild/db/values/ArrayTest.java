...
public class ArrayTest {
  ...
  public void empty_array_is_empty() throws Exception {
    when(() -> valuesDb.arrayBuilder(typeSystem.string()).build().asIterable(SString.class));
  ...
  public void arrays_with_same_elements_have_same_hash() throws Exception {
    ...
    given(sstring = valuesDb.string("abc"));
    given(sstring2 = valuesDb.string("def"));
    when(valuesDb.arrayBuilder(typeSystem.string()).add(sstring).add(sstring2).build().hash());
    thenReturned(valuesDb.arrayBuilder(typeSystem.string()).add(sstring).add(sstring2).build()
        .hash());
  }
  ...
  public void arrays_with_same_elements_but_in_different_order_have_different_hashes()
    ...
    given(sstring2 = valuesDb.string("def"));
    when(valuesDb.arrayBuilder(typeSystem.string()).add(sstring).add(sstring2).build().hash());
    thenReturned(not(valuesDb.arrayBuilder(typeSystem.string()).add(sstring2).add(sstring).build()
        .hash()));
  }
  ...
  public void array_with_one_more_element_have_different_hash() throws Exception {
    ...
    given(sstring = valuesDb.string("abc"));
    given(sstring2 = valuesDb.string("def"));
    when(valuesDb.arrayBuilder(typeSystem.string()).add(sstring).build().hash());
    thenReturned(not(valuesDb.arrayBuilder(typeSystem.string()).add(sstring2).add(sstring).build()
        .hash()));
  }
  ...
  public void array_can_be_read_back() throws Exception {
    ...
    given(array = arrayBuilder.build());
    when(valuesDb.read(typeSystem.array(typeSystem.string()), array.hash()));
    thenReturned(array);
  }
  ...
  public void array_read_back_contains_same_elements() throws Exception {
    ...
    given(arrayBuilder).add(sstring2);
    given(array = arrayBuilder.build());
    when(() -> (valuesDb.read(typeSystem.array(typeSystem.string()), array.hash())).asIterable(
        SString.class));
  ...
  public void array_read_back_has_same_hash() throws Exception {
    ...
    given(arrayBuilder).add(sstring2);
    given(array = arrayBuilder.build());
    when(valuesDb.read(typeSystem.array(typeSystem.string()), array.hash()).hash());
