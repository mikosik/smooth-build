...
public class LambdaTest extends TestingContextImpl {
  ...
  public void creating_lambda_with_body_evaluation_spec_not_equal_result_spec_causes_exception() {
    LambdaSpec lambdaSpec = lambdaSpec(intSpec(), list(stringSpec()));
    assertCall(() -> {
      list(stringExpr());
      lambda(lambdaSpec, boolExpr());
    })
  ...
  public void spec_of_lambda_is_lambda_spec() {
    LambdaSpec lambdaSpec = lambdaSpec(intSpec(), list(stringSpec()));
    stringExpr();
    assertThat(lambda(lambdaSpec, intExpr()).spec())
  ...
  public void body_contains_object_passed_during_construction() {
    ...
    assertThat(lambda(lambdaSpec, body).body())
        .isEqualTo(body);
  }
  ...
  public void lambdas_with_equal_body_and_default_arguments_are_equal() {
    LambdaSpec lambdaSpec = lambdaSpec(intSpec(), list(stringSpec()));
    stringExpr();
    Lambda lambda1 = lambda(lambdaSpec, intExpr());
    stringExpr();
    Lambda lambda2 = lambda(lambdaSpec, intExpr());
    assertThat(lambda1)
        .isEqualTo(lambda2);
  }
  ...
  public void lambdas_with_different_body_are_not_equal() {
    LambdaSpec lambdaSpec = lambdaSpec(intSpec(), list(stringSpec()));
    stringExpr();
    Lambda lambda1 = lambda(lambdaSpec, intExpr(1));
    stringExpr();
    Lambda lambda2 = lambda(lambdaSpec, intExpr(2));
    assertThat(lambda1)
  ...
  public void lambdas_can_be_read_by_hash() {
    LambdaSpec lambdaSpec = lambdaSpec(intSpec(), list(stringSpec()));
    stringExpr();
    Lambda lambda = lambda(lambdaSpec, intExpr());
