...
public class ObjTypeTest {
  @ParameterizedTest
  @MethodSource("names")
  public void name(ObjType type, String name) {
  ...
  @ParameterizedTest
  @MethodSource("names")
  public void quoted_name(ObjType type, String name) {
    assertThat(type.name())
        .isEqualTo(name);
  }
  ...
  @ParameterizedTest
  @MethodSource("names")
  public void to_string(ObjType type, String name) {
  ...
  public void jType(ObjType type, Class<?> expected) {
  ...
  public void array_element(ArrayOType type, ObjType expected) {
  ...
  public void tuple_item(TupleOType type, List<ObjType> expected) {
  ...
  class _evaluation_type {
    @ParameterizedTest
    @MethodSource("types")
    public void order(ValType type) {
    ...
    @ParameterizedTest
    @MethodSource("types")
    public void const_(ValType type) {
    ...
    @ParameterizedTest
    @MethodSource("types")
    public void select(ValType type) {
      assertThat(OBJECT_TYPE_DB.select(type).evaluationType())
          .isEqualTo(type);
    }

    public static ImmutableList<ObjType> types() {
      return TestingObjTypes.VAL_TYPES_TO_TEST;
    }
  }

  private static TupleOType tupleType(ValType... items) {
    return tupleType(list(items));
  }

  private static TupleOType tupleType(ImmutableList<ValType> items) {
    return OBJECT_TYPE_DB.tuple(items);
  }

  private static ConstructOType constructType(ValType... items) {
