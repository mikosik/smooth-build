...
public class ExprBCollisionTest extends TestContext {
  ...
  private List<ExprB> exprBs() {
    ...
    return List.of(
        ...
        tupleB(stringB("def")),
        // expressions
        callB(defFuncB(funcTB(intTB(),stringTB() ), stringB("a")), intB(1)),
        callB(defFuncB(funcTB(intTB(),stringTB() ), stringB("a")), intB(2)),
        callB(defFuncB(funcTB(intTB(),stringTB() ), stringB("b")), intB(1)),
        callB(natFuncB(funcTB(intTB(),intTB() )), intB(1)),
        callB(natFuncB(funcTB(intTB(),intTB() )), intB(2)),
        callB(natFuncB(funcTB(intTB(),stringTB() )), intB(1)),
        combineB(),
        combineB(intB(0)),
        combineB(intB(1)),
        ifFuncB(intTB()),
