...
public class MapBTest extends TestContext {
  ...
  @Nested
  class _equals_hash_hashcode extends ExprBTestCase<MapB> {
    @Override
    protected List<MapB> equalExprs() {
      return list(
    ...
    @Override
    protected List<MapB> nonEqualExprs() {
      return list(
          mapB(arrayB(intB(7)), funcB(list(intTB()), stringB("abc"))),
  ...
  public void invoke_read_back_by_hash_has_same_data() {
    ...
    var readMap = (MapB) bytecodeDbOther().get(mapB.hash());
