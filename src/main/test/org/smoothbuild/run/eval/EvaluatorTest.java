...
public class EvaluatorTest  extends TestingContext {
  ...
  class _func {
    ...
    public void ann_func() throws Exception {
      ...
      var byteFuncS = byteFuncS(className, varS("A"), "myFunc", nList(itemS(varS("A"), "p")));
  ...
  private ObjB evaluate(ObjS objS) {
    return evaluate(objS, nList());
  }

  private ObjB evaluate(ObjS objS, NList<RefableObjS> topRefables) {
